...
import type { ModelClient } from "../utils/get_model_client";
...
// Track active streams with full session data
...
export function registerChatStreamHandlers() {
  ipcMain.handle("chat:stream", async (event, req: ChatStreamParams) => {
    ...
    try {
      ...
      activeStreams.set(req.chatId, streamSession);
      
      // Send session-created event to renderer
...
`;
      ...
      if (testResponse) {
        ...
        const hasUploadedAttachments = req.attachments?.some(
...
`;
        ...
        if (isSummarizeIntent) {
          ...
          chatMessages = [
            {
              role: "user",
              content: `Summarize the following chat: ${formatMessagesForSummary(previousChat?.messages ?? [])}`,
        ...
        const simpleStreamText = async ({
          ...
          const contextWindow = maxOutputTokens; // Store for token budget calculation
          ...
          const streamResult = streamText({
            ...
            onFinish: (response) => {
              ...
              if (typeof totalTokens === "number") {
                ...
                // Send token budget update to renderer
                const session = activeStreams.get(req.chatId);
                if (session) {
                  const tokenBudget = {
                    used: totalTokens,
                    total: contextWindow,
                  ...
                  session.lastTokenUpdate = tokenBudget;
...
[Response cancelled by user]`,
  ...
  // Handler to cancel an ongoing stream with enhanced cleanup
  ipcMain.handle("chat:cancel", async (event, chatId: number) => {
    const session = activeStreams.get(chatId);

    if (session) {
      try {
        // Log session details before cleanup
        const sessionDuration = Date.now() - session.startTime;
        logger.log(
          `Aborting stream session ${session.sessionId} for chat ${chatId}`,
          {
            duration: `${sessionDuration}ms`,
            lastTokenUpdate: session.lastTokenUpdate
              ? `${session.lastTokenUpdate.used}/${session.lastTokenUpdate.total}`
              : "none",
          }
        );
        ...
        // Send session-aborted event to renderer
        safeSend(event.sender, "chat:response:session-aborted", {
          chatId,
          sessionId: session.sessionId,
          reason: "user_cancelled",
        });

        logger.log(`Successfully aborted stream session ${session.sessionId}`);
      } catch (error) {
        logger.error(
          `Error during session abort for chat ${chatId}:`,
          error
        );
        // Continue with cleanup even if abort fails
      }
    } else {
      logger.warn(`No active stream found for chat ${chatId} - may have already completed`);
    }
    ...
    try {
      ...
      logger.error(`Error clearing file uploads state for chat ${chatId}:`, error);
    }

    return true;
  });
  
  // Handler to get active session info (for debugging/monitoring)
  ipcMain.handle("chat:get-session-info", async (event, chatId: number) => {
    const session = activeStreams.get(chatId);
    ...
    return {
      ...
      lastTokenUpdate: session.lastTokenUpdate,
      isActive: !session.abortController.signal.aborted,
    };
  });
  
  // Handler to cleanup all active sessions (for app shutdown)
  ipcMain.handle("chat:cleanup-all-sessions", async (event) => {
    const sessionCount = activeStreams.size;
    
    if (sessionCount === 0) {
      logger.log("No active sessions to cleanup");
      return { cleaned: 0 };
    }
    
    logger.log(`Cleaning up ${sessionCount} active sessions`);
    const cleanedSessions: string[] = [];
    
    for (const [chatId, session] of activeStreams.entries()) {
      try {
        session.abortController.abort();
        cleanedSessions.push(session.sessionId);
        logger.log(`Cleaned up session ${session.sessionId} for chat ${chatId}`);
      } catch (error) {
        logger.error(
          `Error cleaning up session ${session.sessionId} for chat ${chatId}:`,
          error
        );
      }
    }
    ...
    logger.log(`Successfully cleaned up ${cleanedSessions.length} sessions`);
    
    return {
      cleaned: cleanedSessions.length,
      sessionIds: cleanedSessions,
    };
  });
}
