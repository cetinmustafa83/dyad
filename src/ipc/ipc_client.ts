...
import type { DeepLinkData } from "./deep_link_data";
...
export class IpcClient {
  ...
  private constructor() {
    ...
    this.ipcRenderer.on("chat:response:session-created", (data) => {
      if (
        ...
        const { chatId, sessionId } = data as {
          chatId: number;
          sessionId: string;
        };
        const callbacks = this.chatStreams.get(chatId);
  ...
  // Get active session info for a chat (for debugging/monitoring)
  public async getChatSessionInfo(chatId: number): Promise<{
    sessionId: string;
    startTime: number;
    duration: number;
    lastTokenUpdate?: {
    ...
    return this.ipcRenderer.invoke("chat:get-session-info", chatId);
  }

  // Cleanup all active chat sessions (for app shutdown)
  public async cleanupAllChatSessions(): Promise<{
    cleaned: number;
    sessionIds: string[];
  }> {
    return this.ipcRenderer.invoke("chat:cleanup-all-sessions");
  }
  ...
  public async listLocalClaudeCodeModels(): Promise<LocalModel[]> {
    const response = await this.ipcRenderer.invoke(
  ...
  public async getClaudeCodeInstallationInfo(): Promise<{
    installed: boolean;
    path: string | null;
    version: string | null;
  }> {
    return await this.ipcRenderer.invoke("claudecode:get-installation-info");
  }

  public async getClaudeCodeCapabilities(): Promise<{
    mcpServers: string[];
    skills: string[];
    plugins: string[];
    agents: string[];
    ...
    return await this.ipcRenderer.invoke("claudecode:get-capabilities");
  }

  public async selectClaudeCodePath(): Promise<string | null> {
    return await this.ipcRenderer.invoke("claudecode:select-path");
  }

  public async installClaudeCode(params: {
    useSudo: boolean;
  }): Promise<{ success: boolean; output: string; error?: string }> {
    return await this.ipcRenderer.invoke("claudecode:install", params);
  }

  public async updateClaudeCode(): Promise<{
    success: boolean;
    output: string;
    error?: string;
  }> {
    return await this.ipcRenderer.invoke("claudecode:update");
  }
