...
export function ModelPicker() {
  ...
  // Claude Code Models Hook
  const {
    models: claudeCodeModels,
    loading: claudeCodeLoading,
    error: claudeCodeError,
    loadModels: loadClaudeCodeModels,
  } = useClaudeCodeModels();
  ...
  }, [open, loadOllamaModels, loadLMStudioModels, loadClaudeCodeModels]);
  ...
  const getModelDisplayName = () => {
    ...
    if (selectedModel.provider === "claudecode") {
      return (
        claudeCodeModels.find(
          (model: LocalModel) => model.modelName === selectedModel.name,
        )?.displayName || selectedModel.name
      );
    }
  ...
  const hasClaudeCodeModels =
    !claudeCodeLoading && !claudeCodeError && claudeCodeModels.length > 0;
        ...
        <DropdownMenuSub>
          <DropdownMenuSubTrigger className="w-full font-normal">
            <div className="flex flex-col items-start">
              ...
              <span className="text-xs text-muted-foreground">
                Claude Code, LM Studio, Ollama
              </span>
            </div>
          </DropdownMenuSubTrigger>
          <DropdownMenuSubContent className="w-56">
            {/* Claude Code Models SubMenu */}
            <DropdownMenuSub>
              <DropdownMenuSubTrigger
                disabled={claudeCodeLoading && !hasClaudeCodeModels}
              ...
              >
                <div className="flex flex-col items-start">
                  ...
                  {claudeCodeLoading ? (
                    ...
                    <span className="text-xs text-muted-foreground">
                      {claudeCodeModels.length} models
                    </span>
                  )}
                </div>
              </DropdownMenuSubTrigger>
              <DropdownMenuSubContent className="w-56 max-h-100 overflow-y-auto">
                <DropdownMenuLabel>Claude Code Models</DropdownMenuLabel>
                <DropdownMenuSeparator />

                {claudeCodeLoading && claudeCodeModels.length === 0 ? (
                  <div className="text-xs text-center py-2 text-muted-foreground">
                    Loading models...
                  </div>
                ) : claudeCodeError ? (
                  ...
                  <div className="px-2 py-1.5 text-sm">
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-muted-foreground">
                        Configure Claude Code in settings.
                      </span>
                    </div>
                  </div>
                ) : (
                  claudeCodeModels.map((model: LocalModel) => (
                    <DropdownMenuItem
                      key={`claudecode-${model.modelName}`}
                      className={
                        selectedModel.provider === "claudecode" &&
